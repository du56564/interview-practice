package interview.lld.pubsubsystem;

import interview.lld.pubsubsystem.entity.Message;
import interview.lld.pubsubsystem.observer.AlertSubscriber;
import interview.lld.pubsubsystem.observer.NewsSubscriber;
import interview.lld.pubsubsystem.observer.Subscriber;

/*
A Publish-Subscribe (Pub-Sub) system is a messaging pattern where publishers send messages to topics without knowing who will receive them,
and subscribers receive messages by subscribing to those topics. Example: Kafka, Redis Pub/Sub

Requirement:
    In-Memory Pub/Sub
    Functional:
        Multiple Pub/Sub for topic
        Asynchronous delivery of message to active subscriber
        Guarantee message ordering
        Fire and Forget model
        Dynamically subscribe and unsubscribe
    Non-Functional:
        Modularity, Scalability, Extensibility, Reliability

    Entity
        Message (timestamp, payload)
        Topic (add, remove, broadcast to subscriber)
        Subscriber (alert, news)
        PubSubService (createTopic, publish, subscribe, unsubscribe)
